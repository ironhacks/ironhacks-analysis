...
```{r}
df_describe <- ddply(df,~group,summarise,mean=mean(ln_total),sd=sd(ln_total))
df_describe
```
## Descriptive Statistics ( mean and sd ) for ln_exploration

```{r}
df_describe <- ddply(df,~group,summarise,mean=mean(ln_exploration),sd=sd(ln_exploration))
df_describe
```
