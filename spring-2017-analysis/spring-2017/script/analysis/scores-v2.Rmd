...
```{r}
anova_test(data = df, formula = ln_novelty ~ group, type = 3, detailed = TRUE) # type 3 SS should be used in ANCOVA
```
The group  is significant suggesting it is an important predictor of genotypes yield.


```{r}
# Get adjusted means,
adj_means <- emmeans_test(data = df, formula = novelty ~ group)
...
```{r}
emmeans_test(data = df, formula = novelty ~ group, covariate = Q7_Q7_1, p.adjust.method = "fdr")
```
